...
class BajpaiReussPlant:
    ...
    def get_initial_state(self, batch_size):
        ...
        """
        X_init = torch.rand((batch_size, 1), device=self.device) * 2.0 + 0.1   # 0.1 to 2.1 g/L
        S_init = torch.rand((batch_size, 1), device=self.device) * 1.0 + 0.1   # 0.1 to 1.1 g/L
        P_init = torch.zeros((batch_size, 1), device=self.device)              # 0.0 g/L initial product
        V_init = torch.rand((batch_size, 1), device=self.device) * 5.0 + 5.0   # 5.0 to 10.0 L initial volume
        C_L_init = torch.full((batch_size, 1), 0.1, device=self.device)  # Shape: (batch_size, 1)
