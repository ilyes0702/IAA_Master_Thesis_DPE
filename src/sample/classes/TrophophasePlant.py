...
class TrophophasePlant:
    def __init__(self, hyperparam_config):
        ...
        self.mu_max = torch.tensor(0.12, device=self.device)      # [1/h]
        self.Ks = torch.tensor(50.0, device=self.device)         # [mg S/l]
        self.m_S = torch.tensor(23.0, device=self.device)        # [mg S/(g TS h)]
        self.p1 = torch.tensor(0.00047, device=self.device)      # [g TS/(mg S)]
        self.p2 = torch.tensor(200000.0, device=self.device)     # [mg S/l]
    ...
    def get_initial_state(self, batch_size):
        ...
        x1_init = torch.rand((batch_size, 1), device=self.device) * 50.0 + 100.0 
        x2_init = torch.rand((batch_size, 1), device=self.device) * 1000.0 + 500.0 
