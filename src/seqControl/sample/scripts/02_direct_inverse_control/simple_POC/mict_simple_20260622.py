...
class StatefulMambaController(nn.Module):
    ...
    def allocate_inference_states(self, batch_size=1, device="cuda"):
        ...
        # Mamba requires specific shapes to hold its internal convolutional/recurrent buffers
    ...
    def step(self, y_t_single, y_next_single, conv_state, ssm_state):
        ...
        # 1. Force both inputs to be flat vectors: [Batch=1]
        y_t_flat = y_t_single.reshape(-1)
        y_next_flat = y_next_single.reshape(-1)

        # 2. Combine them into a clean 2D tensor: [Batch=1, Features=2]
        x = torch.stack([y_t_flat, y_next_flat], dim=-1)
        
        # 3. Project features up to model dimension -> Keeps shape at [1, 32]
        x = self.input_proj(x) 
        
        # 🟢 4. CRITICAL FIX: Squeeze an explicit sequence dimension of 1 into the middle
        # This converts the shape from [1, 32] to [1, 1, 32] -> [Batch, Seq, Feature]
        x_3d = x.unsqueeze(1)
        
        # 5. Execute single-step recurrent update via official custom CUDA kernels
        # The internal kernel will now read hidden_states.shape[1] as exactly 1!
        x_out_3d, conv_state, ssm_state = self.core.step(x_3d, conv_state, ssm_state)
        
        # 6. Flatten back down to 2D [1, 32] for the output projection layer
...
def simulate_controller_stateful(model, plant, dt, steps=50, dirname="plots", x_scaler=None, y_scaler=None, ref_freq=1.0, ref_amplitude=0.18):
    ...
    y = torch.tensor([[0.0]], device=device) # Shape: [Batch=1, 1]
    y_log, u_log, ref_log = [], [], []

    # 🟢 1. Initialize the internal memory buffers once before starting
    ...
    for t in range(steps):
        ...
        # 🟢 3. Convert only the CURRENT step to single-element tensors
        # 🟢 CHANGE: Use 2D tensors [Batch=1, Feature=1] instead of 3D [[[ value ]]]
        ...
        # 🟢 4. Forward step using the state memory (Modifies caches automatically)
        with torch.no_grad():
            u_norm_tensor, conv_state, ssm_state = model.step(
                y_t_norm, y_next_norm, conv_state, ssm_state
            )
        
        u_norm_np = u_norm_tensor.cpu().numpy() # Shape: [1, 1]
    ...
    # --- Plotting code remains exactly identical ---
