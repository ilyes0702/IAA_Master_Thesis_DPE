...
class NonlinearHydraulicPlant:
    ...
    def _dynamics(self, x, u, t):
        ...
        # Standard interacting tank root-law hydraulics wrapped with safety clamping 
        # to prevent negative square roots during transient exploration
...
def generate_data_parallel(plant, seq_len=50, num_sequences=5000):
    dt = plant.dt
    lambd = 2.0  
    p = 2.5  # Input scaling factor mapped within non-biological limits [0, 5.0]
    ...
    plant.reset_time()
    # Initialize state matrix with nominal mechanical starting levels: Tank 1 = 2.0m, Tank 2 = 1.5m
...
def simulate_controller_stateful(model, plant, dt, ref_sequence, dirname="plots", x_scaler=None, y_scaler=None):
    ...
    for t in range(steps):
        ...
        y_current = plant.output_characteristic(x_state[:, 1], plant.current_time)
...
def simulate_constant_controller(model, plant, dt, steps=50, dirname="plots", x_scaler=None, y_scaler=None, ref_value=1.5):
    ...
    for t in range(steps):
        ...
        # Physical constraints clamp: Inflow pump limitation bounded between 0 and 5.0 m^3/h
        u = np.clip(u, 0.0, 5.0)
...
def main():
    ...
    # --- Closed-loop constant reference tracking ---
    simulate_constant_controller(
        model, plant, dt, steps=50,
        x_scaler=x_scaler, y_scaler=y_scaler,
        ref_value=1.5  # Fixed reference target below the maximum flow saturation limit
    )
